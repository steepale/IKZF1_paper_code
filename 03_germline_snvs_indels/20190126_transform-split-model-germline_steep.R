...
# Processed data
######################
proc_file <- './data/processed-data_steep.txt'
...
df_grouped <- df %>% dplyr::select(dependents_grouped, response)
df_ungrouped <- df %>% dplyr::select(dependents_ungrouped, response)
...
########################
set.seed(123)
train_rows <- createDataPartition(df_grouped$SNP_MA, p = 0.70, list = FALSE)
...
#########################
set.seed(123)
train_rows <- createDataPartition(df_ungrouped$SNP_MA, p = 0.70, list = FALSE)
...
# To load the model
rpart_fit <- readRDS(paste0("./models/",date,"_cart-model-germline-snps_steep.rds"))
...
write.table(cm_out,
            file=paste0("./data/",date,"_test-confusion-matrix-cart-germline-snps_steep.txt"),
...
#' * SNPs from 22 F1 normal tissues were concatenated. Data was fairly homogenous and was particioned using the createDataPartition function in caret. 70% (n=199,094) of the data was sued to generate a training set, while 30% (n=85,325) were used as a test set.
